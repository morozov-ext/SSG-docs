...
Базовый синтаксис поддерживается практически всеми [диалектами](../introduction/#mark-1) Markdown. Некоторые особенности могут отличаться,
...
А чтобы перенести текст на следующую строку, используйте **разрыв строки** одним из способов:
...
{{< callout context="tip" title="Лучшие практики" icon="outline/info-circle">}}
-  Выберите один набор символов для каждого начертания. Например, `_` для курсивного и `**` для жирного.
...
Чтобы создать **нумерованный список (ordered list)**, поставьте число с точкой (`1.`, `2.` и т. д.) и пробел на каждой строке списка.
...
Чтобы создать **ненумерованный список (unordered list)**, поставьте звездочку (`*`), или плюс (`+`), или минус (`-`) и пробел на каждой строке списка.
...
Список можно сделать **вложенным**, если поставить четыре пробела или табуляцию перед маркером (`1.`, `*`, и т. д.).
...
По такому же принципу вставляйте в список параграфы, цитаты, изображения, блоки кода.
...
| 1. Вот пункт один<br>2. Вот пункт два<br>&nbsp;&nbsp;&nbsp;&nbsp;\!\[Выхухоль](/img/vih.png)<br>3. Вот пункт три  | <ol><li>Вот пункт один<li>Вот пункт два</li><img src="/images/vih.png" width="128"><li>Вот пункт три |
...
круглыми скобками (`[Текст ссылки](URL)`). Для ссылки на электронную почту в этом варианте нужно перед URL добавить `mailto:` (`[текст ссылки](mailto:URL)`).
...
Если вам понадобится вставить один из символов `*_{}[]<>()#+-.!| как текст, может возникнуть проблема. Markdown использует
